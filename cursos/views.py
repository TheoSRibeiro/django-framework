...
class CursoAPIView(generics.RetrieveUpdateDestroyAPIView):  # GET, POST, PUT E DELETE - NECESSARIO PASSAR ID - retorna 1 elemento
    queryset = Curso.objects.all()
    serializer_class = CursoSerializer
...
class AvaliacoesAPIView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        if self.kwargs.get('curso_pk'): # verificar se esta passando o id do curso na url (kwargs captura o valor do id passado)
            return self.queryset.filter(curso_id=self.kwargs.get('curso_pk')) #retorna somente a avaliacao do curso selecionado
